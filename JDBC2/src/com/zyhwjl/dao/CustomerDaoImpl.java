...
public class CustomerDaoImpl extends BaseDao implements CustomerDao{
    ...
    public Customer getCustomerById(Connection conn, int id) {
        ...
        try {
            return query1(customer.getClass(),conn,sql,id);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    @Override
    public List<? extends Customer> getAll(Connection conn) {
        String sql = "select id,name,email,birth from customers where 1 = ?";
        Customer customer = new Customer();
        try {
            return querylist(customer.getClass(),conn,sql,1);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
