...
import com.zyhwjl.myssm.myexception.CustomException;
...
//    private final CustomersImpl customersDao = new CustomersImpl();
    ...
    private String editP(Integer id,String name,String password) throws Exception {
        customerService.updateCustomer(new Customers(id,name,password));
        return "redirect:customer.do";
    }

    private String editG(Integer id,HttpServletRequest req) throws Exception {
        if(id!=null){
...
//            HttpSession session = req.getSession();
    ...
    private String delete(Integer id) throws Exception {
        customerService.delCustomer(id);
        return "redirect:customer.do";
    }

    private String add(String name,String password) throws Exception {
        customerService.addCustomer(new Customers(name,password));
        return "redirect:customer.do";
    }

    private String index(String keyword, Integer pageN, String sign,HttpSession session,HttpServletRequest req) throws Exception {
        if(pageN==null){
            pageN = 1;
        }
        int pageS = 5 ;
